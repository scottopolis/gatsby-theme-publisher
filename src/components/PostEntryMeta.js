...
const PostEntryMeta = ({ post, location = '' }) => (
  ...
  {post.categories && location === 'blog' ? post.categories.nodes.map(category => <span key={category.id} className="bg-gray-200 rounded p-2 text-xs float-right xl:order-last xl:text-center ml-1 xl:ml-0 xl:mb-1">{category.name}</span>) : null }
